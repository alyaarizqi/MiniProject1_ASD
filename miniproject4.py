...
class SistemPesanKue:
    ...
    def sorting_pesanan(self, key1, key2, ascending=True):
        ...
        pesanan_list.sort(key=lambda x: (str(x.get(key1)).lower(), str(x.get(key2)).lower()), reverse=not ascending)
    ...
    #Implementasi untuk fitur searching
    def jump_search(self, pesanan_list, key, value):
        length = len(pesanan_list)
        step = int(length ** 0.5)
        prev = 0
        while pesanan_list[min(step, length) - 1][key] < value:
            prev = step
            step += int(length ** 0.5)
            if prev >= length:
                return -1
        while pesanan_list[prev][key] < value:
            prev += 1
            if prev == min(step, length):
                return -1
        if pesanan_list[prev][key] == value:
            return prev
        return -1

    def search_pesan(self):
        print("=================================")
        print("        MENU CARI PESANAN        ")
        print("=================================")
        print("1. Cari berdasarkan ID Pesanan")
        print("2. Cari berdasarkan Nama Kue")
        print("3. Cari berdasarkan Nama Pelanggan")
        option = input("Pilihan Anda: ")

        if option == "1":
            pesan_id = int(input("Masukkan ID Pesanan yang ingin Anda cari: "))
            pesanan_list = self.get_pesanan_list()
            pesanan_list.sort(key=lambda x: x['id'])  
            result_index = self.jump_search(pesanan_list, 'id', pesan_id)
            if result_index != -1:
                print(f"ID Pesanan {pesan_id} ditemukan.")
                self.baca_pesanan(pesan_id)
            else:
                print(f"ID Pesanan {pesan_id} tidak ditemukan.")
        elif option == "2":
            nama_kue = input("Masukkan Nama Kue yang ingin Anda cari: ").lower()
            pesanan_list = self.get_pesanan_list()
            pesanan_list.sort(key=lambda x: x['nama_kue'].lower())  
            pesanan_list = [pesan for pesan in pesanan_list if pesan['nama_kue'].lower() == nama_kue]
            if pesanan_list:
                print(f"Nama Kue '{nama_kue}' ditemukan.")
                self.baca_pesanan(pesanan_list[0]['id'])
            else:
                print(f"Nama Kue '{nama_kue}' tidak ditemukan.")
        elif option == "3":
            nama_pelanggan = input("Masukkan Nama Pelanggan yang ingin Anda cari: ").lower()
            pesanan_list = self.get_pesanan_list()
            pesanan_list.sort(key=lambda x: x['nama_pelanggan'].lower())  
            pesanan_list = [pesan for pesan in pesanan_list if pesan['nama_pelanggan'].lower() == nama_pelanggan]
            if pesanan_list:
                print(f"Nama Pelanggan '{nama_pelanggan}' ditemukan.")
                self.baca_pesanan(pesanan_list[0]['id'])
            else:
                print(f"Nama Pelanggan '{nama_pelanggan}' tidak ditemukan.")
        else:
            print("Pilihan tidak valid!")


    def get_pesanan_list(self):
        """Mengembalikan daftar pesanan dalam bentuk list."""
        pesanan_list = []
        current = self.head
        while current:
            pesanan_list.append(current.data)
    ...
    def quick_sort(self, data, key, ascending=True):
        if len(data) <= 1:
            ...
            # Konversi string menjadi huruf kecil untuk penyortiran yang tidak sensitif terhadap kasus
            less_than_pivot = [x for x in data[1:] if str(x.get(key)).lower() <= str(pivot).lower()]
            greater_than_pivot = [x for x in data[1:] if str(x.get(key)).lower() > str(pivot).lower()]
    ...
    def buat_pesanan(self, nama_kue, jenis_kue, harga_kue, topping_kue, ukuran_kue, jumlah_pesanan, nama_pelanggan, tanggal_pemesanan, alamat_pelanggan):
        ...
        if pilihan == "1":
            ...
            self.sorting_pesanan('id', 'id', True)  
        elif pilihan == "2":
            ...
            ...
            if current.data['id'] == pesan_id:
                ...
                while True:
                    ...
                    if pilihan == "1":
                        ...
                        print("━━━━━━━━━━━━━ ⋆⋅☆⋅⋆ ━━━━━━━━━━━━")
                        self.tampilkan_dalam_pretty_table()
                        break
                    else:
                        print("━━━━━━━━━━━━━ ⋆⋅☆⋅⋆ ━━━━━━━━━━━━")
                        print("     Pilihan tidak valid!      ")
    ...
    def tampilkan_dalam_pretty_table(self):
        table = PrettyTable()
        table.field_names = ["ID Pesanan", "Nama Kue", "Jenis Kue", "Harga Kue", "Topping Kue", "Ukuran Kue", "Jumlah Pesanan", "Nama Pelanggan", "Alamat Pelanggan", "Tanggal Pemesanan"]
        current = self.head
        while current:
            pesan = current.data
            table.add_row([
    ...
    def operasi_pesan(self):
        while True:
            ...
            if operasi == "1":
                ...
                ascending = sorting == "1"
                if sorting == "2":
                    self.sorting_pesanan(operasi, 'id', ascending=False)  
                else:
                    self.sorting_pesanan(operasi, 'id', ascending)  
                self.tampilkan_dalam_pretty_table()
            elif operasi == "6":
                self.search_pesan()
            elif operasi == "7":
...
sistem_pemesanan.buat_pesanan("Red Velvet Cake", "Birthday Cake", 250000, "Cream Cheese", "Sedang", 2, "Alya Rizqi", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "Jl. Kemangi No. 123")
sistem_pemesanan.buat_pesanan("Chocolate Cupcake", "Cupcake", 30000, "Chocolate Chips", "Kecil", 12, "Najla Nayla", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "Jl. Juanda No. 456")
sistem_pemesanan.buat_pesanan("Blueberry Tart", "Kue Tart", 35000, "Blueberry Jam", "Besar", 1, "Diva Andini", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "Jl. Antasari No. 789")
sistem_pemesanan.buat_pesanan("Lapis Legit", "Kue Tradisional", 150000, "Cinnamon Powder", "Sedang", 1, "Alyssa Dwiana", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "Jl. Cendana No. 101")
sistem_pemesanan.buat_pesanan("Croissant", "Roti", 20000, "Butter", "Besar", 6, "Nova Fauziah", datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "Jl.  No. 111")

sistem_pemesanan.operasi_pesan()

sistem_pemesanan.tampilkan_dalam_pretty_table()